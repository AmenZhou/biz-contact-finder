#!/usr/bin/env python3
"""
Export Queens & Brooklyn Accountants to KMZ for Google Maps (Final with ratings)
"""
...
INPUT_CSV = DATA_DIR / "queens_brooklyn_accountants_final.csv"
OUTPUT_KML = DATA_DIR / "queens_brooklyn_accountants.kml"
OUTPUT_KMZ = DATA_DIR / "queens_brooklyn_accountants.kmz"


def get_coordinates_from_serper(name: str, address: str) -> tuple:
    """
    Get coordinates for accountant using Serper API
    """
    import requests
    import time
...
def create_kmz(df: pd.DataFrame):
    """Create KMZ file with accountant markers"""
    print("=" * 80)
    ...
    kml = Kml()
    kml.document.name = "Queens & Brooklyn Accountants"
    kml.document.description = f"""
Accountants in Queens and Brooklyn
...
"""
    ...
    print()
    print(f"✓ Processed {total} accountants")
...
def main():
    ...
    print(f"   ✓ Loaded {len(df)} accountants")
    print()
