#!/usr/bin/env python3
"""
Export Bronx Census Tract Median Household Income to KMZ
...
OUTPUT_KML = EXPORTS_DIR / "bronx_income.kml"
...
def download_census_tracts() -> Path:
    """Download TIGER/Line shapefiles for Bronx census tracts"""
...
def fetch_income_data() -> pd.DataFrame:
    ...
    # Build API request for Bronx
    params = {
        'get': 'NAME,B19013_001E',  # B19013_001E = Median Household Income
        'for': 'tract:*',
        'in': f'state:{NY_STATE_FIPS}+county:{BRONX_FIPS}'
    }
...
def merge_geo_and_income(shapefile_path: Path, income_df: pd.DataFrame) -> gpd.GeoDataFrame:
    ...
    gdf = gdf[gdf['COUNTYFP'] == BRONX_FIPS].copy()
    print(f"   Loaded {len(gdf)} tracts for Bronx")
...
def create_kmz(gdf: gpd.GeoDataFrame) -> None:
    ...
    kml = Kml()
    kml.document.name = "Bronx Median Household Income"
...
"""
    ...
    # Create folder for Bronx
    bronx_folder = kml.newfolder(name=f"Bronx ({len(gdf)} tracts)")
    ...
    for idx, row in gdf.iterrows():
        ...
        # Create polygon
        pol = bronx_folder.newpolygon(name=placemark_name)
...
def main():
    ...
    print("=" * 60)
    print("BRONX: MEDIAN HOUSEHOLD INCOME MAP")
    print("=" * 60)
