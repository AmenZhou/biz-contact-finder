#!/usr/bin/env python3
"""
Contact Enrichment Script for Top Buildings
...
def load_top_buildings(n: int = TOP_N_BUILDINGS) -> List[str]:
    ...
    for file in DATA_DIR.glob("*_merchants.csv"):
...
def enrich_building(building_name: str) -> Dict:
    ...
    # Enrich merchants
    merchants_file = DATA_DIR / f"{building_name}_merchants.csv"
    ...
    # Enrich lawyers
    lawyers_file = DATA_DIR / f"{building_name}_lawyers.csv"
    ...
    contacts_file = DATA_DIR / f"{building_name}_building_contacts.csv"
...
def main():
    ...
    print()
    print("Next step: Run export_tenants_to_kmz.py to regenerate KMZ with enriched data")
    print("="*60)
