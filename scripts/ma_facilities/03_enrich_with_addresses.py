#!/usr/bin/env python3
"""
Enrich Massachusetts facilities with addresses using HERE Geocoding & Search API
"""
...
INPUT_CSV = DATA_DIR / "ma_facilities.csv"
OUTPUT_CSV = DATA_DIR / "ma_facilities_with_addresses.csv"
...
def search_here_places(query: str, location: str = "Massachusetts") -> List[Dict]:
    """
    ...
    # Massachusetts area center (roughly)
    params = {
        "q": query,
...
def parse_here_result(item: Dict) -> Optional[Dict]:
    ...
    if "contacts" in item:
        ...
        if contacts and len(contacts) > 0:
            ...
            if "www" in contact:
                www_list = contact["www"]
                if www_list and len(www_list) > 0:
                    result["website"] = www_list[0].get("value")
...
def needs_coordinates(row: pd.Series) -> bool:
    """Check if row needs coordinates"""
...
def enrich_with_address(row: pd.Series) -> pd.Series:
    ...
    # Skip if address is already valid and has coordinates
    if not needs_enrichment(row['address']) and not needs_coordinates(row):
        return row

    # Build search query
    name = row['name']
    location_search = row.get('location', 'Massachusetts')
    ...
    if items and len(items) > 0:
        ...
        if parsed:
            # Update fields
            if parsed.get('address') and needs_enrichment(row['address']):
                row['address'] = parsed['address']
                print(f"    New: {parsed['address'][:60]}")

            if parsed.get('phone') and (pd.isna(row['phone']) or not row['phone']):
                row['phone'] = parsed['phone']

            if parsed.get('website') and (pd.isna(row['website']) or not row['website']):
                row['website'] = parsed['website']

            if parsed.get('latitude') and needs_coordinates(row):
                row['latitude'] = parsed['latitude']
                row['longitude'] = parsed['longitude']
                print(f"    Coords: {parsed['latitude']}, {parsed['longitude']}")
        else:
...
def main():
    """Main execution"""
    print("=" * 80)
    print("ENRICHING MASSACHUSETTS FACILITIES WITH ADDRESSES (HERE API)")
    print("=" * 80)
    print()
    ...
    missing_coords = df.apply(needs_coordinates, axis=1)
    ...
    # Enrich each record
    needs_work = invalid_addresses | missing_coords
    print(f"Enriching {needs_work.sum()} records with HERE API...")
    print()
    ...
    final_with_coords = ~df.apply(needs_coordinates, axis=1)
    ...
    print(f"   With coordinates: {final_with_coords.sum()}")
    print()
    print("Improvement:")
