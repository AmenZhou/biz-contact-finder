...
Uses Serper.dev API to find all medical practices and enrich contact information
"""
...
OUTPUT_CSV = DATA_DIR / "queens_brooklyn_doctors.csv"
...
# Search terms for medical practices
SEARCH_TERMS = [
    "doctors office",
    "medical clinic",
    "physician office",
...
def parse_serper_results(results: Dict, search_query: str, location: str) -> List[Dict]:
    """
    Parse Serper results to extract doctor office information
    """
    doctors = []
    seen_names = set()
    ...
    if "places" in results:
        for place in results["places"]:
            ...
            # Filter for medical practices
            title_lower = name.lower()
            if not any(keyword in title_lower for keyword in [
                "doctor", "dr.", "medical", "clinic", "health", "care",
                "physician", "practice", "md", "medicine", "hospital"
            ]):
                continue
    ...
    if "organic" in results:
        for result in results["organic"]:
            ...
            if not any(keyword in title_lower or keyword in snippet for keyword in [
                "doctor", "dr.", "medical", "clinic", "health", "care",
                "physician", "practice", "md", "medicine"
            ]):
                continue
...
def enrich_contact_with_serper(doctor: Dict) -> Dict:
    """
    Enrich doctor office contact info using Serper + OpenAI
    """
    if not doctor.get("website"):
        return doctor

    # Search for contact information
    contact_query = f"{doctor['name']} contact email phone"
    ...
    if "organic" in results:
        for result in results["organic"][:3]:  # Check top 3 results
            ...
            if not doctor.get("email"):
                ...
                if email_match:
                    doctor["email"] = email_match.group()
    ...
    if openai_client and "organic" in results:
        try:
            ...
            prompt = f"""Extract contact information for this medical office:
Name: {doctor['name']}
...
Return only valid JSON, no explanation."""
            ...
            if extracted.get("email") and not doctor.get("email"):
                doctor["email"] = extracted["email"]
            if extracted.get("phone") and not doctor.get("phone"):
                doctor["phone"] = extracted["phone"]
            if extracted.get("contact_name"):
                doctor["contact_name"] = extracted["contact_name"]
...
def scrape_doctors():
    ...
    print("=" * 80)
    print("QUEENS & BROOKLYN DOCTOR'S OFFICE SCRAPER")
    print("=" * 80)
    print()

    # Load progress
    progress = load_progress()
    all_doctors = progress.get("found_doctors", {})
    completed = set(progress.get("completed_searches", []))

    print(f"📊 Progress: {len(completed)} searches completed, {len(all_doctors)} doctors found")
    print()
    ...
    for idx, (search_term, location) in enumerate(remaining, 1):
        ...
        # Parse results
        doctors = parse_serper_results(results, search_term, location)
        print(f"   ✓ Found {len(doctors)} doctor offices")

        # Add to collection (deduplicate by name + address)
        for doctor in doctors:
            key = f"{doctor['name']}|{doctor['address']}".lower()
            if key not in all_doctors:
                all_doctors[key] = doctor
        ...
        if idx % 10 == 0:
            ...
            progress["found_doctors"] = all_doctors
            save_progress(progress)
            print(f"   💾 Progress saved ({len(all_doctors)} total doctors)")
    ...
    print()
    print("=" * 80)
    print(f"✅ Scraping complete! Found {len(all_doctors)} unique doctor offices")
    print("=" * 80)
    print()
...
def enrich_contacts(doctors: List[Dict]) -> List[Dict]:
    """Enrich contact information for all doctors"""
    print()
    print("=" * 80)
    ...
    for idx, doctor in enumerate(doctors, 1):
        print(f"[{idx}/{total}] Enriching: {doctor['name']}")

        # Skip if already has good contact info
        has_email = bool(doctor.get("email"))
        has_phone = bool(doctor.get("phone"))
        ...
        enriched_doctor = enrich_contact_with_serper(doctor)
        enriched.append(enriched_doctor)

        # Show what was found
        if enriched_doctor.get("email") and not has_email:
            print(f"   ✓ Found email: {enriched_doctor['email']}")
        if enriched_doctor.get("phone") and not has_phone:
            print(f"   ✓ Found phone: {enriched_doctor['phone']}")
...
def export_to_csv(doctors: List[Dict]):
...
def main():
    ...
    try:
        ...
        enriched_doctors = enrich_contacts(doctors)

        # Step 3: Export to CSV
        export_to_csv(enriched_doctors)
