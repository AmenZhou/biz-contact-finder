#!/usr/bin/env python3
"""
Extract addresses from doctor office websites using LLM
"""
...
OUTPUT_CSV = DATA_DIR / "queens_brooklyn_doctors_with_addresses_llm.csv"
...
def extract_address_with_llm(website_text: str, office_name: str) -> Dict:
    ...
    prompt = f"""You are extracting contact information from a doctor's office website.
...
def process_record(row: pd.Series, index: int, total: int) -> pd.Series:
    """Process a single doctor office record"""
...
def main():
    """Main execution"""
    print("=" * 80)
    print("LLM-BASED ADDRESS EXTRACTION FROM DOCTOR OFFICE WEBSITES")
    print("=" * 80)
    print()
