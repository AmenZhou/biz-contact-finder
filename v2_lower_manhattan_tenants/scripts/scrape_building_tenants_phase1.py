#!/usr/bin/env python3
"""
Phase 1: Office Building Tenant Scraper - Web Scraping + OpenAI Only
...
COVERAGE_REPORT = DATA_DIR / 'scraping_coverage_report.csv'
...
def load_and_filter_buildings() -> List[Dict]:
    """
    Load office buildings and filter for commercial/office only.

    Returns:
        List of commercial office building dictionaries
    """
    ...
    # Exclude: apartments, residential, hotel, construction
    exclude_types = ['apartment', 'residential', 'hotel', 'construction', 'church', 'school']

    commercial_buildings = []
    for building in all_buildings:
        building_type = (building.get('building_type') or '').lower()

        # Skip residential buildings
        if any(ex in building_type for ex in exclude_types):
            continue

        # Include office/commercial or unknown types with high floor count
        if ('office' in building_type or
            'commercial' in building_type or
            int(building.get('estimated_tenants', 0)) >= 10):
            commercial_buildings.append(building)

    print(f"  Commercial/office buildings: {len(commercial_buildings)}")
...
def save_merchants_csv(building_address: str, merchants: List[Dict]):
    ...
    output_file = DATA_DIR / f"{filename}_merchants.csv"
...
def save_building_contacts_csv(building_address: str, contacts: List[Dict]):
    ...
    filename = re.sub(r'[^\w\s-]', '', building_address.replace(',', ''))
    filename = re.sub(r'\s+', ' ', filename).strip()
    output_file = DATA_DIR / f"{filename}_building_contacts.csv"
...
def save_lawyers_csv(building_address: str, lawyers: List[Dict]):
    ...
    filename = re.sub(r'[^\w\s-]', '', building_address.replace(',', ''))
    filename = re.sub(r'\s+', ' ', filename).strip()
    output_file = DATA_DIR / f"{filename}_lawyers.csv"
...
def main():
    """Main function."""
    print("="*60)
    print("PHASE 1: TENANT SCRAPER - WEB SCRAPING + OPENAI")
    print("="*60)
    ...
    # Remove this limit for full run
    buildings = buildings[:10]
    print(f"\n⚠ Testing mode: Processing first {len(buildings)} buildings only\n")
    ...
    print(f"{'='*60}")
    print("Review the coverage report to decide if you want to run Phase 2")
    print("(Phase 2 = Fill gaps with Google Places API)")
